...
/* ═══════════════════════════════════════════════════
   EXERCISE REVIVAL — Subject → Class → Chapter → Section popup → Questions
   ═══════════════════════════════════════════════════ */
const ER_SECTIONS = [
  { key: "exercise_revival_questions", label: "Questions", icon: "📝", color: "text-emerald-600", bg: "bg-emerald-100", border: "border-emerald-300" },
  { key: "exercise_revival_explanation", label: "Explanation", icon: "💡", color: "text-blue-600", bg: "bg-blue-100", border: "border-blue-300" },
  { key: "exercise_revival_source_line", label: "Source Line", icon: "📖", color: "text-purple-600", bg: "bg-purple-100", border: "border-purple-300" },
  { key: "exercise_revival_hidden_concepts", label: "Hidden Concepts & Q&A", icon: "🔍", color: "text-orange-600", bg: "bg-orange-100", border: "border-orange-300" },
  { key: "exercise_revival_mcq_practice", label: "MCQ Practice Zone", icon: "🎯", color: "text-rose-600", bg: "bg-rose-100", border: "border-rose-300" },
];

function ExerciseRevivalFlow() {
  const [step, setStep] = useState("subject");
  const [selectedSubject, setSelectedSubject] = useState(null);
  const [selectedClass, setSelectedClass] = useState(null);
  const [selectedChapter, setSelectedChapter] = useState(null);
  ...
  const breadcrumb = () => { const p = ["Exercise Revival"]; if (selectedSubject) p.push(selectedSubject.name); if (selectedClass) p.push(selectedClass.name); if (selectedChapter) p.push(selectedChapter.name); if (selectedSection) p.push(selectedSection.label); return p; };
  const typeLabel = { MCQ: "MCQ", FILL_BLANK: "Fill in Blank", MATCH: "Match the Following" };
  const diffColor = { EASY: "bg-green-100 text-green-700", MEDIUM: "bg-yellow-100 text-yellow-700", HARD: "bg-red-100 text-red-700" };
...
function FeatureContentInner() {
  ...
  if (featureType === "exercise_revival") return <ExerciseRevivalFlow />;
