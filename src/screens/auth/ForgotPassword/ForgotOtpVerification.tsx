...
export const OTPVerification = ({ navigation, route }: OTPVerificationProps) => {
    ...
    verifyOTP(
      { email: normalizedEmail, otp: otpString },
      {
        onSuccess: (data) => {
          ...
          navigation.navigate('ResetPassword', { email: normalizedEmail, resetToken });
        },
        onError: (error: any) => {
          const message =
            error?.userMessage ||
  ...
  const maskedEmail = String(email || '').replace(/(.{2})(.*)(@.*)/, '$1***$3');
...
const styles = StyleSheet.create({
  ...
  contactText: { fontWeight: '700', color: '#1a1a1a' },
  ...
  otpInputFocused: { borderColor: '#FED93A', backgroundColor: '#fffbf0' },
  otpInputFilled: { borderColor: '#FED93A', backgroundColor: '#fffbf0' },
  ...
  resendLink: { fontSize: 14, color: '#FED93A', fontWeight: '700', textDecorationLine: 'underline' },
